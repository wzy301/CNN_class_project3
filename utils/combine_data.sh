...
if $has_trials; then
  # we are going to create an trials file in the destdir
  for in_dir in $*; do
    if [ -f $in_dir/trials ]; then
      cat $in_dir/trials
    fi
  done  > $dest/trials
  echo "$0: combined trials"
else
  echo "$0 [info]: not combining trials as it does not exist"
fi
